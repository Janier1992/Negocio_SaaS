...
import {
  ...
  logAuditEgreso,
  type EgresoRow,
} from "@/services/finanzas/egresos";
...
export default function Finanzas() {
  ...
  const [prevResumen, setPrevResumen] = useState<{
    ingresos_mes: number;
    egresos_mes: number;
    balance_mes: number;
    cogs: number;
  ...
  const [egresosByCategoria, setEgresosByCategoria] = useState<
  ...
  useEffect(() => {
    try {
      localStorage.setItem(
        "finanzas_egresos_filters",
        JSON.stringify({ filtroDesde, filtroHasta, filtroCat }),
      );
    } catch {}
  ...
  useEffect(() => {
    ...
    const fetchResumen = async (background: boolean) => {
      ...
      try {
        ...
        if (ventasRes.error) {
          ...
          ingresos = (ventasRes.data || []).reduce(
        ...
        if (ventasPrevRes.error) {
          ...
          prevIngresos = (ventasPrevRes.data || []).reduce(
            (sum: number, v: any) => sum + Number(v.total || 0),
            0,
          );
        }
        ...
        const prevCogs = comprasPrevRows.reduce(
        ...
        const egresosManualPrev = await fetchEgresos({
          empresaId,
          desde: prev.desde,
          hasta: prev.hasta,
        });
        const egresosManualPrevSum = egresosManualPrev.reduce(
        ...
        const egTrend = Array.from(dailyMap.entries()).map(([label, v]) => ({
          label,
          total: v.egresos,
        }));
        ...
        const catArr = Array.from(catMap.entries()).map(([categoria, total]) => ({
          categoria,
          total,
        }));
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "compras", filter: `empresa_id=eq.${empresaId}` },
        () => fetchResumen(true),
      )
      .on("postgres_changes", { event: "*", schema: "public", table: "compras_detalle" }, () =>
        fetchResumen(true),
      )
      .on(
        "postgres_changes",
  ...
  const submitEgreso = async () => {
    ...
    try {
      ...
      await logAuditEgreso("create", empresaId, {
        id: row.id,
        monto: row.monto,
        fecha: row.fecha,
      ...
      <div className="space-y-6">
        <div>
          ...
          <div className="mt-4 flex items-center gap-3 flex-wrap">
            ...
            <Button onClick={() => setFormOpen((o) => !o)}>
              {formOpen ? "Ocultar formulario" : "Registrar egreso"}
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <Card>
            <CardHeader>
              <CardTitle>
                Ingresos ({period === "hoy" ? "Hoy" : period === "semana" ? "Semana" : "Mes"})
              </CardTitle>
              <CardDescription>Ventas del periodo seleccionado</CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-3xl font-bold">
                {currencyFormatter.format(Number(resumen?.ingresos_mes || 0))}
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader>
              <CardTitle>
                Egresos ({period === "hoy" ? "Hoy" : period === "semana" ? "Semana" : "Mes"})
              </CardTitle>
              <CardDescription>COGS + egresos operacionales</CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-3xl font-bold">
                {currencyFormatter.format(Number(resumen?.egresos_mes || 0))}
              </p>
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                Balance ({period === "hoy" ? "Hoy" : period === "semana" ? "Semana" : "Mes"})
              </CardTitle>
              <CardDescription>Ingresos - Egresos del periodo</CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-3xl font-bold">
                {currencyFormatter.format(Number(resumen?.balance_mes || 0))}
              </p>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <p className="text-2xl font-bold">
                {currencyFormatter.format(
                  Number((resumen?.ingresos_mes || 0) - (resumen?.egresos_mes || 0)),
                )}
              </p>
              <div className="text-xs mt-1">
                {(() => {
                  ...
                  const prev = Number(
                    (prevResumen?.ingresos_mes || 0) - (prevResumen?.egresos_mes || 0),
                  );
                  const diff = cur - prev;
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-2xl font-bold">
                {currencyFormatter.format(
                  Number((resumen?.ingresos_mes || 0) - (resumen?.cogs || 0)),
                )}
              </p>
        ...
        <Dialog open={formOpen} onOpenChange={(v) => setFormOpen(v)}>
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                <label className="text-sm" htmlFor="monto">
                  Monto
                </label>
                <Input
                  id="monto"
                  ...
                  aria-invalid={!!formErrors.monto}
                />
                {formErrors.monto && (
                  <div className="text-xs text-destructive mt-1">{formErrors.monto}</div>
                )}
              </div>
              <div>
                ...
                />
                {formErrors.fecha && (
                  <div className="text-xs text-destructive mt-1">{formErrors.fecha}</div>
                )}
              </div>
              <div>
                <label className="text-sm" htmlFor="categoria">
                  Categoría
                </label>
                <Input
                  id="categoria"
                  placeholder="Servicios, Impuestos, Renta..."
                  value={categoria}
                  onChange={(e) => setCategoria(e.target.value)}
                  aria-invalid={!!formErrors.categoria}
                />
                {formErrors.categoria && (
                  <div className="text-xs text-destructive mt-1">{formErrors.categoria}</div>
                )}
              </div>
            ...
            <div className="mt-4 flex items-center gap-3">
              ...
              <Button variant="outline" onClick={exportEgresosCSV}>
                Exportar CSV
              </Button>
            </div>
          </DialogContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Filtra y explora tus egresos; exporta PDF por categoría.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                <label className="text-sm" htmlFor="filtroDesde2">
                  Filtrar desde
                </label>
                <Input
                  id="filtroDesde2"
              ...
              <div>
                <label className="text-sm" htmlFor="filtroHasta2">
                  Filtrar hasta
                </label>
                <Input
                  id="filtroHasta2"
              ...
              <div>
                <label className="text-sm" htmlFor="filtroCat2">
                  Categoría
                </label>
                <Input
                  id="filtroCat2"
              ...
              <div className="flex items-end">
                <Button variant="outline" onClick={() => exportEgresosPDF(filteredEgresos)}>
                  Exportar PDF
                </Button>
              </div>
            </div>
            <div className="mt-4 border rounded">
              ...
              {filteredEgresos.length === 0 && (
                <div className="px-3 py-2 text-xs text-muted-foreground">
                  Sin egresos para los filtros seleccionados.
                </div>
              )}
              {filteredEgresos.map((r) => (
                <div key={r.id} className="grid grid-cols-12 gap-2 px-3 py-2 text-xs border-t">
                  ...
                  <div className="col-span-3 truncate" title={r.descripcion || ""}>
                    {r.descripcion}
                  </div>
                  <div className="col-span-3 text-right">
                    {currencyFormatter.format(Number(r.monto || 0))}
                  </div>
                </div>
              ))}
              {filteredEgresos.length > 0 && (
                <div className="grid grid-cols-12 gap-2 px-3 py-2 text-xs bg-muted/30 border-t">
                  ...
                  <div className="col-span-3 text-right font-medium">
                    {currencyFormatter.format(
                      filteredEgresos.reduce((s, r) => s + Number(r.monto || 0), 0),
                    )}
                  </div>
                </div>
              )}
