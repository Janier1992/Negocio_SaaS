...
const SUPABASE_URL = (import.meta.env.VITE_SUPABASE_URL ||
  import.meta.env.VITE_NEW_SUPABASE_URL) as string | undefined;
const SUPABASE_ANON_KEY = (import.meta.env.VITE_SUPABASE_ANON_KEY ||
  import.meta.env.VITE_NEW_SUPABASE_PUBLISHABLE_KEY) as string | undefined;
...
async function notifyTechnicalTeam(status: number | null, error: string | null, attempts: number) {
  ...
  if (TECH_ALERT_WEBHOOK) {
    try {
      await fetch(TECH_ALERT_WEBHOOK, {
        method: "POST",
...
export async function checkAuthConnectivity(opts?: {
  retries?: number;
  baseDelayMs?: number;
  timeoutMs?: number;
}): Promise<AuthHealth> {
  ...
  if (!envOk.isConfigured) {
    ...
    return {
      ok: false,
      url: envOk.url,
      anonKeyPresent: !!envOk.anonKey,
      reachable: false,
      status: null,
      error: "Entorno incompleto",
    };
  }
  ...
  for (let i = 0; i < retries; i++) {
    ...
    log.warn(`Fallo de conectividad a Auth (status=${res.status ?? "n/a"})`, {
      attempt,
      error: res.error,
    });
