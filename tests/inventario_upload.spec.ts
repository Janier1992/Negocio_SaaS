...
vi.mock("@/integrations/supabase/newClient", () => {
  ...
  const builder: any = {
    ...
    eq: vi.fn((_col: string, _val: any) =>
      Promise.resolve({ data: existingProductos, error: null }),
    ),
...
describe("Carga masiva de productos: parseo y actualización de precio", () => {
  it("actualiza precios existentes con formatos de moneda locales", async () => {
    ...
    const res = await uploadProductosCore(
      hoisted.sheetRows,
      "empresa-1" as any,
      (await import("@/integrations/supabase/newClient")).supabase as any,
      categorias,
      proveedores,
    );
  ...
  it('soporta decimales con coma y encabezado "Precio"', async () => {
    ...
    expect(hoisted.recordedUpdates.length).toBe(3);
    const preciosActualizados = hoisted.recordedUpdates.map((u) => u.payload.precio);
    expect(preciosActualizados).toEqual([1200.0, 2500.5, 8500.75]);
