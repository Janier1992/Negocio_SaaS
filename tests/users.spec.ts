...
const mockRpc = vi.fn();
const mockAuth = { getSession: vi.fn(() => Promise.resolve({ data: { session: { access_token: 'token' } } })), getUser: vi.fn(() => Promise.resolve({ data: { user: { id: 'admin-user' } } })) };
...
vi.mock('@/integrations/supabase/newClient', () => {
  ...
  const builder = {
    ...
    eq: vi.fn(() => builder),
    maybeSingle: vi.fn(() => Promise.resolve({ data: { id: 'target-user', empresa_id: 'empresa-1', rol: 'empleado' }, error: null })),
  } as any;
